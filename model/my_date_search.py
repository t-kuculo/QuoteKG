...
class _ExactLanguageSearch:
    ...
    def search_parse(self, shortname, text, settings):
        ...
        return list(zip(substrings, [i[0] for i in parsed]))
...
class DateSearchWithDetection:
    ...
    @apply_settings
    def search_dates(self, text, language, settings=None):
        """
        ...
        if language not in self.available_language_map.keys():
            language="en"

        return {'Language': language, 'Dates': self.search.search_parse(language, text, 
                                                                                  settings=settings)}