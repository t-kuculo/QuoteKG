...
pattern_brackets2 = re.compile(r'\([^\)]*\)') # https://stackoverflow.com/questions/640001/how-can-i-remove-text-within-parentheses-with-a-regex
...
print(cleanText("\"W1  W2 [2] [3] <ref>test</ref> W3 <ref mode='ccc'/> W4 <ref mode='aaa'>xyz</ref> W5 <small>"), flush=True)
...
languages = ["si","is","sa","vi","da","ka","hi","uz","eu","ku","ro","kn","sq","ml","no","cy","be","te","ur","th","gl","gu","simple","sr","sah","ta","la","ja","nl","ko","hu","li","sv","id","nn","su","el","hy","hr","ar","bg","et","zh","eo","lt","sl","az","fi","ca","tr","bs","sk","he","uk","fr","es","pt","de","fa","cs","ru","pl","it","en"]
languages.reverse()

QKG = Namespace("http://quotekg.l3s.uni-hannover.de/resource/")
...
file = "/home/gottschalk/quotekg/rdf/corpus_evaluation_fast_0.8.pkl"
...
def processTemplateContext(g, templateObject, contextURI, page_language):
    if isinstance(templateObject,str):
        ...
        if text:
            g.add((contextURI, SO.text, Literal(text, page_language)))
...
for completeQuote in corpus.completeQuotes.values():
    ...
    for lang, quotes in completeQuote.quotes.items():
        ...
        for quote in quotes:
                ...
                for entity in quote.entities:
                    if entity.wikidata_id:
                        ...
                        if wikidata_id not in entity_dict:
                            ...
                            rdf_entity.wikidata_id = wikidata_id
                            entity_dict[wikidata_id] = rdf_entity
                        else:
                            rdf_entity = entity_dict[wikidata_id]
            ...
            if quote.footnotes:
                for footnote in quote.footnotes: # footnotes are text-only contexts
                    ...
                    if text:
                        ...
                        g.add((contextURI, SO.text, Literal(text, quote.page_language)))
                        g.add((contextURI, SO.inLanguage, Literal(quote.page_language, datatype=XSD.language)))
                        g.add((quotationTextURI, QKG.hasContext, contextURI))
                        contextId += 1
            ...
            if text in existing_quotes[completeEntity.wikidata_id][lang]: # Remove duplicates. TODO: Which of the duplicates to keep? What if quotation is empty?
                continue
            
            existing_quotes[completeEntity.wikidata_id][lang].add(text)
            
            quotationTextURI = URIRef(QKG)+"QuotationText" + str(quotationId)
            quotationId += 1
            g.add((quotationURI, QKG.hasText, quotationTextURI))
            g.add((quotationTextURI, RDF.type, QKG.QuotationText))
            # text = cleanText(quote.quote, isQuote = True)
            
            # TODO: check if text is non-empty
            g.add((quotationTextURI, SO.text, Literal(cleanText(quote.quote), lang)))
            g.add((quotationTextURI, SO.inLanguage, Literal(lang, datatype=XSD.language)))
            ...
            for section_title in quote.section_titles:
                ...
                if section_title:
                    if len(section_title) >= 3 and section_title != 'Quotes':
                        if quote.page_language not in person.wikiquoteIds or person.wikiquoteIds[quote.page_language] != section_title:
                            g.add((quotationTextURI, DCTERMS.description, Literal(section_title, quote.page_language)))
...
for line in wikidata_to_dbpedia_file:
    ...
    if wikidata_id in used_wikidata_ids:
        if wikidata_id not in wikidata_to_dbpedia:
            wikidata_to_dbpedia[wikidata_id] = []
        if dbpedia_id.startswith("http://dbpedia.org/resource/"): # English
            wikidata_to_dbpedia[wikidata_id].append(dbpedia_id)
        else:
            for lang in languages:
...
for entity in entity_dict.values():
    ...
    if entity.wikidata_id in wikidata_to_dbpedia:
        for dbpedia_url in wikidata_to_dbpedia[entity.wikidata_id]:
